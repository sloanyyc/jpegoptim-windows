...
#include <jni.h>
JNIEXPORT jint JNICALL Java_com_sloan_jpegoptim_JpegOptim_exec
(JNIEnv* env, jobject o, jbyteArray bytearray)
{
	char chars[10240] = { 0 };
	char* args[100] = { 0 };
	char* pargs = chars;
	...
	if (chars_len < 10200) {
		...
		size_t arg_len = strlen(pargs);
		size_t arg_count = 0;
		while (arg_len < 90 && arg_len > 0) {
			args[arg_count++] = pargs;
			pargs = pargs+(arg_len + 1l);
			arg_len = strlen(pargs);
		}
		args[arg_count] = 0;
		main((int)arg_count, args);
	}
	return 0;
}
